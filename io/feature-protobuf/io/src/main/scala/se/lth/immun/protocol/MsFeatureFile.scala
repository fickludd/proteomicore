...
object MsFeatureFile {
	...
	def read(f:File, verbose:Boolean) = {
		...
		val rtMult = rtMap.getUnit match {
			case RtUnit.MINUTE => 60.0
			case RtUnit.SECOND => 1.0
		}
		val rts = rtMap.getRtList.map(_ * rtMult)
		
		val features = new ArrayBuffer[MsFeature]
		var i = 0
		while (parseAndCodeSize(r, verbose)) {
			features += readMsFeature(r, rts, rtMult, i)
			i += 1
		}
	...
	def readMsFeature(
		...
		val hills = f.getHillList.map(toMsHill(_, rtMap, rtMult))
		val rtApex = 
			if (f.hasRtApex) f.getRtApex * rtMult 
	...
	def toMsHill(hill:Hill, rtMap:Seq[Double], rtMult:Double):MsHill = {
		...
		MsHill(
				...
				hill.getMz,
				hill.getMzErr,
				if (hill.hasFwhm) Some(hill.getFwhm.toDouble * rtMult) else None,
				rtApex,
				intensityApex,
				intensity)
	}
