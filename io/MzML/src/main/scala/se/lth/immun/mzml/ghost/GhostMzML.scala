...
object GhostMzML {
	...
	def fromFile(r:XmlReader, ignoreTIC:Boolean = true, binaryFileChannel:FileChannel = null):GhostMzML = {
		val x = new GhostMzML
		
		var dh = new MzMLDataHandlers(
				ns => x.spectra.sizeHint(ns), 
				s => x.spectra += GhostSpectrum.fromSpectrum(s), 
				nc => x.chromatograms.sizeHint(nc), 
				c => {
					if (!ignoreTIC || !c.cvParams.exists(_.accession == Ghost.TIC_ACC))
						x.chromatograms += GhostChromatogram.fromChromatogram(c)
				})
...
class GhostMzML {
	...
	val spectra = new ArrayBuffer[GhostSpectrum]
	val chromatograms = new ArrayBuffer[GhostChromatogram]
}